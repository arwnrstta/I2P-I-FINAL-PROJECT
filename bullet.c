...
bool update_bullet(Bullet * bullet, enemyNode * enemyList, Map * map){
    ...
    */
    ...
    if (tile_x < 0 || tile_y < 0 || tile_x >= map->row || tile_y >= map->col || 
        map->map[tile_x][tile_y] == WALL) {
        return true;
    }
    ...
    while(cur != NULL){
        ...
        /*
            [TODO Hackathon 3-2] Hit the enemy DONE
