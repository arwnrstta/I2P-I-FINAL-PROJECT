...
bool updateEnemy(Enemy * enemy, Map * map, Player * player){
    ...
    if(enemy->status == DYING){
        ...
        */ 
        enemy->death_animation_tick = (enemy->death_animation_tick + 1) % 64;
        if (enemy->death_animation_tick == 63) {
            enemy->death_animation_tick = 62;
            return true;
        }
...
void drawEnemy(Enemy * enemy, Point cam){
    ...
    if(enemy->status == ALIVE){
        ...
        */
        offset_x = 16 * (enemy->death_animation_tick / (64 / 8));
        offset_y = 16;
        ...
        if (enemy->type == slime) {
            al_draw_tinted_scaled_rotated_bitmap_region(
                ...
                16, 16, //width n height
                al_map_rgb(tint_red, 255, 255), //color
                0, 0, //pivot
                dx, dy, //destination
                TILE_SIZE / 16, TILE_SIZE / 16, //scale
                0, flag); //rotation n flip
        }
    }
...
static bool isCollision(Point enemyCoord, Map* map){
    ...
    /* 
        ...
        if(!isWalkable(map->map[...][...])) return true;
    */
    int enemyCoord_x1 = enemyCoord.x / TILE_SIZE;
    int enemyCoord_y1 = enemyCoord.y / TILE_SIZE;
    int enemyCoord_x2 = (enemyCoord.x + TILE_SIZE - 1) / TILE_SIZE;
    int enemyCoord_y2 = (enemyCoord.y + TILE_SIZE - 1) / TILE_SIZE;

    if (!isWalkable(map->map[enemyCoord_y1][enemyCoord_x1])) return true;
    if (!isWalkable(map->map[enemyCoord_y2][enemyCoord_x2])) return true;
    if (!isWalkable(map->map[enemyCoord_y1][enemyCoord_x2])) return true;
    if (!isWalkable(map->map[enemyCoord_y2][enemyCoord_x1])) return true;
